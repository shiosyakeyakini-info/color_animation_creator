...
{
    [CustomEditor(typeof(ColorHueAnimator))]
    public class ColorHueAnimatorEditor : UnityEditor.Editor
    {
        ...
        static ColorHueAnimatorEditor()
        {
            // ドロップダウン用ラベル配列（末尾に「カスタム」を追加）
        ...
        public override void OnInspectorGUI()
        {
            ...
            var animator = (ColorHueAnimator)target;

            // ヘッダー
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("色相アニメーション作成", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox(
                "このコンポーネントは、liltoonシェーダーの任意の色プロパティの色相をRadial Menuで変更します。\n" +
                "ビルド時に自動的にアニメーションクリップを生成し、AnimatorとMenuに追加します。",
                MessageType.Info);
            EditorGUILayout.Space();
            ...
            // プロパティの存在確認と型チェック
            if (targetRenderer.objectReferenceValue != null && materialIndex.intValue >= 0)
            {
                var renderer = (Renderer)targetRenderer.objectReferenceValue;
                if (materialIndex.intValue < renderer.sharedMaterials.Length)
                {
                    var material = renderer.sharedMaterials[materialIndex.intValue];
                    if (material != null)
                    {
            ...
            EditorGUILayout.PropertyField(hueSteps, new GUIContent("色相サンプル数"));
            EditorGUILayout.HelpBox(
                "色相のサンプル数。数値が大きいほど滑らかですが、ファイルサイズが増加します。\n" +
                "推奨値: 24～36",
                MessageType.Info);
            ...
            EditorGUILayout.PropertyField(parameterName, new GUIContent("パラメータ名"));
            ...
            {
                EditorGUILayout.HelpBox("設定は正常です。", MessageType.Info);
            }
